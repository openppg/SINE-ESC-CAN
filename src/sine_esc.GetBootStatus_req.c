...
uint32_t sine_esc_GetBootStatusRequest_encode(struct sine_esc_GetBootStatusRequest* msg, uint8_t* buffer
    ...
    uint32_t bit_ofs = 0;
    memset(buffer, 0, SINE_ESC_GETBOOTSTATUS_REQUEST_MAX_SIZE);
    _sine_esc_GetBootStatusRequest_encode(buffer, &bit_ofs, msg, 
#if CANARD_ENABLE_TAO_OPTION
    tao
#else
    true
...
#if CANARD_ENABLE_TAO_OPTION
    if (transfer->tao && (transfer->payload_len > SINE_ESC_GETBOOTSTATUS_REQUEST_MAX_SIZE)) {
        return true; /* invalid payload length */
    }
#endif
    uint32_t bit_ofs = 0;
    if (_sine_esc_GetBootStatusRequest_decode(transfer, &bit_ofs, msg,
#if CANARD_ENABLE_TAO_OPTION
    transfer->tao
#else
    true
...
struct sine_esc_GetBootStatusRequest sample_sine_esc_GetBootStatusRequest_msg(void) {
    struct sine_esc_GetBootStatusRequest msg;
