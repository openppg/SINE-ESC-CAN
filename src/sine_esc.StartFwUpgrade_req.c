...
uint32_t sine_esc_StartFwUpgradeRequest_encode(struct sine_esc_StartFwUpgradeRequest* msg, uint8_t* buffer
    ...
    uint32_t bit_ofs = 0;
    memset(buffer, 0, SINE_ESC_STARTFWUPGRADE_REQUEST_MAX_SIZE);
    _sine_esc_StartFwUpgradeRequest_encode(buffer, &bit_ofs, msg, 
#if CANARD_ENABLE_TAO_OPTION
    tao
#else
    true
...
#if CANARD_ENABLE_TAO_OPTION
    if (transfer->tao && (transfer->payload_len > SINE_ESC_STARTFWUPGRADE_REQUEST_MAX_SIZE)) {
        return true; /* invalid payload length */
    }
#endif
    uint32_t bit_ofs = 0;
    if (_sine_esc_StartFwUpgradeRequest_decode(transfer, &bit_ofs, msg,
#if CANARD_ENABLE_TAO_OPTION
    transfer->tao
#else
    true
...
struct sine_esc_StartFwUpgradeRequest sample_sine_esc_StartFwUpgradeRequest_msg(void) {
    struct sine_esc_StartFwUpgradeRequest msg;

    msg.hardware_id = (uint16_t)random_bitlen_unsigned_val(16);
    msg.filse_size_kb = (uint16_t)random_bitlen_unsigned_val(16);
    msg.file_crc = (uint16_t)random_bitlen_unsigned_val(16);
    return msg;
}
#endif
