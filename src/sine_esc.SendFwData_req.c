...
uint32_t sine_esc_SendFwDataRequest_encode(struct sine_esc_SendFwDataRequest* msg, uint8_t* buffer
    ...
    uint32_t bit_ofs = 0;
    memset(buffer, 0, SINE_ESC_SENDFWDATA_REQUEST_MAX_SIZE);
    _sine_esc_SendFwDataRequest_encode(buffer, &bit_ofs, msg, 
#if CANARD_ENABLE_TAO_OPTION
    tao
#else
    true
...
#if CANARD_ENABLE_TAO_OPTION
    if (transfer->tao && (transfer->payload_len > SINE_ESC_SENDFWDATA_REQUEST_MAX_SIZE)) {
        return true; /* invalid payload length */
    }
#endif
    uint32_t bit_ofs = 0;
    if (_sine_esc_SendFwDataRequest_decode(transfer, &bit_ofs, msg,
#if CANARD_ENABLE_TAO_OPTION
    transfer->tao
#else
    true
...
struct sine_esc_SendFwDataRequest sample_sine_esc_SendFwDataRequest_msg(void) {
    struct sine_esc_SendFwDataRequest msg;

    msg.index = (uint16_t)random_bitlen_unsigned_val(16);
    for (size_t i=0; i < 256; i++) {
        msg.data[i] = (uint8_t)random_bitlen_unsigned_val(8);
    }
    return msg;
}
#endif
